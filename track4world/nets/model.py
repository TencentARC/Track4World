...
def refine_confidence_with_geometric_consistency(
    flow2d_c, flow3d, points, visconf_maps_e,
    sensitivity=5.0
):
    """
    Refine visibility/confidence maps by enforcing geometric consistency
    between 2D optical flow and aligned 3D scene flow.

    The core idea is to compare:
        (1) 3D displacement induced by 2D optical flow (s_geo), and
        (2) globally aligned and high-quality 3D scene flow (s_aligned),

    and down-weight confidence values at locations where the two are
    geometrically inconsistent. This helps suppress unreliable regions
    caused by occlusions, flow errors, or depth ambiguities.

    Args:
        flow2d_c (Tensor):
            [B, T-1, H, W, 2]
            Original 2D optical flow from frame t to t+1.

        flow3d (Tensor):
            [B, T-1, H, W, 3]
            Globally aligned and high-quality 3D scene flow (output of the
            alignment module).

        points (Tensor):
            [B, T, H, W, 3]
            3D point cloud sequence for all frames.

        visconf_maps_e (Tensor):
            [B, T-1, H, W, 2]
            Original visibility/confidence maps (e.g., forward/backward confidence).

        sensitivity (float):
            Controls how aggressively geometric inconsistency is penalized.
            Larger values lead to stronger suppression for small errors.

    Returns:
        refined_visconf (Tensor):
            [B, T-1, 2, H, W]
            Geometry-aware refined confidence maps.
    """

    B, T_minus_1, H, W, _ = flow2d_c.shape

    # --- 1. Compute flow-induced 3D displacement s_geo ---
    # p_cur: 3D points at time t
    # p_next: 3D points at time t+1
    p_cur = points[:, :-1].reshape(-1, H, W, 3)
    p_next = points[:, 1:].reshape(-1, H, W, 3)

    flow2d_flat = flow2d_c.reshape(-1, H, W, 2)

    # s_aligned_flat: aligned 3D displacement (scene flow)
    s_aligned_flat = flow3d.reshape(-1, H, W, 3) - p_cur

    # Normalize flow coordinates to [-1, 1] for grid_sample
    # (same normalization convention as the original implementation)
    norm_coords = flow2d_flat.clone()
    norm_coords[..., 0] = 2.0 * norm_coords[..., 0] / (W - 1) - 1.0
    norm_coords[..., 1] = 2.0 * norm_coords[..., 1] / (H - 1) - 1.0

    # Sample next-frame 3D points using 2D optical flow as guidance
    p_next_permuted = p_next.permute(0, 3, 1, 2)  # [N, 3, H, W]
    p_next_sampled = F.grid_sample(
        p_next_permuted,
        norm_coords,
        mode='bilinear',
    ...
    # s_geo: 3D displacement induced purely by 2D optical flow
    s_geo = p_next_sampled - p_cur

    # --- 2. Compute geometric residual ---
    # Large residual indicates inconsistency between 2D-guided motion
    # and globally aligned 3D motion (e.g., due to occlusion or flow errors)
    diff = torch.norm(s_geo - s_aligned_flat, dim=-1, keepdim=True)  # [N, H, W, 1]

    # --- 3. Convert residual to consistency weight ---
    # Exponential decay maps geometric error to [0, 1]
    # Larger error -> smaller weight
    consistency_weight = torch.exp(-sensitivity * diff)

    # --- 4. Refine confidence maps ---
    visconf_flat = visconf_maps_e.reshape(-1, H, W, 2)

    # Apply geometry-aware reweighting
    # (assumes confidence channels are multiplicative)
    refined_conf_flat = visconf_flat * consistency_weight
...
class Track4World(nn.Module):
    ...
    def switch_to_original_backbone(self):
        ...
        This function initializes the backbone network using publicly available
    ...
    def forward_point(
        ...
        """
        Unified feature and geometry extraction interface.

        - for_flow: If False, only extract and return geometric outputs (Pure Point mode).
                    If True, additionally extract flow-related features and return a tuple.
        - num_tokens: If provided, dynamically adjust resolution based on token count.
                    If None, keep original resolution aligned to a 14x14 patch grid.
        """
        ...
        # -----------------------------------------
        # 1. Dynamic / static resolution control
        # -----------------------------------------
        ...
        with torch.no_grad():
            # -----------------------------------------
            # 2. Backbone feature and geometry extraction
        ...
        # -----------------------------------------
        # 4. Pure Point mode (skip flow computation)
        # -----------------------------------------
        if not for_flow:
            points_out = points.permute(0, 2, 3, 1)
            world_points_out = world_points.permute(0, 2, 3, 1)
            if mask.shape[1] == 1:
                mask = mask.squeeze(1)
        ...
        with torch.autocast(device_type=image_14.device.type, dtype=torch.float16):
            ...
            if self.use_3d:
                ...
                fmaps3d_detail = torch.zeros(
                    (points.shape[0], self.flow3d_dim, flow_H, flow_W)
                ).to(image_reshaped.device)

            # Downsample point map for flow correlation computation
    ...
    def infer_pair(
        ...
        # -------------------------------------------------------------
        flow2d = flows_e[None].to(torch.float16).cuda() + grid_xy
        visconf_maps_e = visconf_maps_e[None].to(torch.float16)
        ...
        # -------------------------------------------------------------
        # Geometry processing in float16 for numerical stability
        # -------------------------------------------------------------
        with torch.autocast(device_type=self.device.type, dtype=torch.float16):
