...
class Pi3X(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.use_multimodal = False#use_multimodal
    ...
    def load_checkpoint(self, path):
        ...
        for k, v in state_dict.items():
            if k.startswith('backbone.'):
                k = k.replace('backbone.', '', 1) # 只替换第一个
                
            new_state_dict[k] = v
        # for k, v in state_dict.items():
        #     if k.startswith('model.'):
        #         k = k.replace('model.', '', 1) # 只替换第一个
        ...
        if len(missing) > 0:
            real_missing = [k for k in missing if 'backbone' in k] 
            if len(real_missing) > 0:
                print(f"[ERROR] Still missing backbone keys! Example: {real_missing[:3]}")
            else:
                print(f"[Info] Missing keys (likely safe heads): {missing[:5]} ...")
                
        if len(unexpected) > 0:
            print(f"[Info] Unexpected keys in ckpt (ignored): {unexpected[:5]} ...")
            ...
            with torch.amp.autocast(device_type='cuda', enabled=False):
                ...
                if rays is not None:
                    ...
                    if intrinsics is None:
                        ...
                        pix = torch.from_numpy(get_pixel(H, W).T.reshape(H, W, 3)).to(device)[None].repeat(B, 1, 1, 1)
                ...
                depths_masks = (normalized_depths > 0)
    ...
    def forward_head(self, hidden, pos, B, N, H, W, patch_h, patch_w):
        ...
        xy, z = self.point_head(ret_point[:, self.patch_start_idx:], patch_h=patch_h, patch_w=patch_w)
        camera_poses = self.camera_head(ret_camera[:, self.patch_start_idx:], patch_h, patch_w).reshape(B, N, 4, 4)
        metric = self.metric_head(ret_metric).reshape(B).exp()
        # conf
        ...
        with torch.amp.autocast(device_type='cuda', enabled=True, dtype=torch.float32):
            ...
            t = camera_poses[..., :3, 3] #* metric.view(B, 1)

            camera_poses = torch.cat(
                [
                    torch.cat(
                        [camera_poses[..., :3, :3], t.unsqueeze(-1)],
                        dim=-1
                    ),
                    camera_poses[..., 3:4, :]   # 保留最后一行
                ],
                dim=-2
            )
        ...
        for i in range(len(self.decoder)):
            ...
            if self.use_multimodal:
                if i in [1, 9, 17, 25, 33] and use_pose_mask.sum() > 0:
                    ...
                    hidden_slice = hidden[..., self.patch_start_idx:, :]
                    hidden_slice = hidden_slice + poses_feat * use_pose_mask.view(B, N, 1, 1)
                    hidden = torch.cat(
                        [
                            hidden[..., :self.patch_start_idx, :],
                            hidden_slice
                        ],
                        dim=2
                    )
