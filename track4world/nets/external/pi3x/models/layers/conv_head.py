...
class ConvHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, image: torch.Tensor = None, patch_h: int = None, patch_w: int = None):
        ...
        if isinstance(self.output_block, nn.ModuleList):
            ...
            #         with torch.autocast(device_type=x.device.type, dtype=x.dtype):
            #             return block(x)
            #     output.append(torch.utils.checkpoint.checkpoint(block_forward, x, use_reentrant=False))
            output = [torch.utils.checkpoint.checkpoint(block, x, use_reentrant=False) for block in self.output_block]
        else:
            # def block_forward(x):
            #     with torch.autocast(device_type=x.device.type, dtype=x.dtype):
            #         return self.output_block(x)
            # output = torch.utils.checkpoint.checkpoint(block_forward, x, use_reentrant=False)
