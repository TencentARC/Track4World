...
class Video3DData:
    ...
    video: torch.Tensor  # B,S,C,H,W
    videodepth: torch.Tensor  # B,S,1,H,W
    depths_mask: torch.Tensor  # B,S,1,H,W
    trajs: torch.Tensor  # B,S,N,2
    visibs: torch.Tensor  # B,S,N
    intrs: torch.Tensor  # B,3,3
...
def collate_fn_train_3d(batch):
    ...
    depths_mask = torch.stack([b.depths_mask for b, _ in batch], dim=0)
    metric_scale = torch.tensor([b.metric_scale for b, _ in batch])
    intrs = torch.stack([b.intrs for b, _ in batch], dim=0)
    c2w = torch.stack([b.c2w for b, _ in batch], dim=0)
