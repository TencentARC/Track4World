...
import track4world.utils.py

EPS = 1e-6
COLORMAP_FILE = "track4world/utils/bremm.png"
...
def oned2inferno(d, norm=True, do_colorize=False):
    ...
    if norm:
        d = track4world.utils.basic.normalize(d)
...
def get_feat_pca(feat, valid=None):
    ...
    reduced_emb = track4world.utils.basic.normalize(reduced_emb) - 0.5
...
class SummWriter(object):
        ...
        if frame_ids is not None:
            for s in range(S):
                vis[:, s] = draw_text_on_vis(
                    vis[:, s], track4world.utils.basic.strnum(frame_ids[s]), top=20
                )
    ...
    def summ_rgb(
        ...
        if frame_id is not None:
            vis = draw_text_on_vis(
                vis, track4world.utils.basic.strnum(frame_id), top=20, shadow=shadow
            )
    ...
    def summ_oneds(
        ...
        if norm:
            im = track4world.utils.basic.normalize(im)
    ...
    def summ_feats(
        ...
        if feats.ndim == 6: # B, S, C, D, H, W
            ...
            if valids is None:
                ...
                feats = track4world.utils.basic.reduce_masked_mean(
        ...
        if not pca:
            ...
            # PCA
            feats_packed = track4world.utils.basic.pack_seqdim(feats, B)
            if valids is None:
            ...
            feats_pca = track4world.utils.basic.unpack_seqdim(feats_pca_packed, B)
