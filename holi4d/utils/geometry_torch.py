...
from .geometry_numpy import solve_optimal_focal_shift, solve_optimal_shift, solve_optimal_focal
...
def recover_global_focal_shift(
    ...
    uv_lr = F.interpolate(uv.permute(2, 0, 1).unsqueeze(0), downsample_size, mode='nearest')[0].permute(1, 2, 0)  # (H_d, W_d, 2)
    uv_lr_np = uv_lr.cpu().numpy().reshape(-1, 2)

    # Downsample points
    points_all = points_all.reshape(B * S, H, W, 3).permute(0, 3, 1, 2)  # (B*S, 3, H, W)
    points_all_lr = F.interpolate(points_all, downsample_size, mode='nearest').permute(0, 2, 3, 1)  # (B*S, H_d, W_d, 3)
    points_all_lr = points_all_lr.reshape(B, S, -1, 3)  # (B, S, N, 3)

    if mask_all is not None:
        mask_all = mask_all.reshape(B * S, 1, H, W)
        mask_all_lr = F.interpolate(mask_all.float(), downsample_size, mode='nearest').reshape(B, S, -1) > 0  # (B, S, N)
    else:
        mask_all_lr = torch.ones(B, S, downsample_size[0] * downsample_size[1], dtype=torch.bool, device=device)

    results_focal, results_shift = [], []

    for b in range(B):
        points_b = points_all_lr[b][mask_all_lr[b]]  # (N_valid, 3)
        uv_b = np.tile(uv_lr_np[None], (S, 1, 1))[mask_all_lr[b].cpu().numpy()]  # (N_valid, 2)
        ...
        if focal is None:
            shift_np, focal_np = solve_optimal_focal_shift(uv_b, points_b.cpu().numpy())
            results_focal.append(torch.tensor([focal_np], device=device, dtype=dtype))
        else:
            focal_b = focal[b].item()
            shift_np = solve_optimal_shift(uv_b, points_b.cpu().numpy(), focal_b)
