...
def weighted_mean(
    ...
    if w is None:
        ...
        weighted_sum = (x * w).mean(dim=dim, keepdim=keepdim)
        weights_mean = w.mean(dim=dim, keepdim=keepdim).add(eps)
...
def harmonic_mean(
    ...
    """
    Compute (weighted) harmonic mean of a tensor.
    H = 1 / mean(1 / x)
    """
    if w is None:
        ...
        inv_weighted_mean = weighted_mean(
            x.add(eps).reciprocal(), w, dim=dim, keepdim=keepdim, eps=eps
        )
        return inv_weighted_mean.add(eps).reciprocal()
...
def geometric_mean(
    ...
    """
    Compute (weighted) geometric mean of a tensor.
    G = exp(mean(log(x)))
    """
    if w is None:
        ...
        w = w.to(x.dtype)
        # Calculate weighted mean of log values
        log_weighted_mean = weighted_mean(
            x.add(eps).log(), w, dim=dim, keepdim=keepdim, eps=eps
        )
        return log_weighted_mean.exp()
...
def normalized_view_plane_uv(
    ...
    Coordinates range:
    - Top-left: (-width / diagonal, -height / diagonal)
    - Bottom-right: (width / diagonal, height / diagonal)
    """
...
def gaussian_blur_2d(
    ...
    """
    Apply 2D Gaussian blur to a tensor.
    """
...
def angle_diff_vec3(v1: torch.Tensor, v2: torch.Tensor, eps: float = 1e-12):
    ...
    cross_prod = torch.cross(v1, v2, dim=-1).norm(dim=-1) + eps
    dot_prod = (v1 * v2).sum(dim=-1)
    return torch.atan2(cross_prod, dot_prod)
...
def point_map_to_depth_legacy(points: torch.Tensor):
    ...
    # A: (..., H * W * 2, 2)
    A = torch.stack(
        [points[..., :2], -uv.expand_as(points[..., :2])], dim=-1
    ).flatten(-4, -2)
    ...
    inv_M = torch.inverse(M + 1e-6 * torch.eye(2).to(A))
    At_b = (A.transpose(-2, -1) @ b[..., None])
...
def view_plane_uv_to_focal(uv: torch.Tensor):
    """Estimate focal length from UV coordinates."""
...
def recover_focal_shift(
    ...
    for i in range(points.shape[0]):
        # Filter valid points
        if mask is None:
            points_lr_i_np = points_lr_np[i]
            uv_lr_i_np = uv_lr_np
        else:
            points_lr_i_np = points_lr_np[i][mask_lr_np[i]]
            uv_lr_i_np = uv_lr_np[mask_lr_np[i]]
        ...
        if focal is None:
            ...
            optim_shift_i = solve_optimal_shift(
...
def recover_global_focal_shift(
    points_all: torch.Tensor,  # shape: (B, S, H, W, 3)
    mask_all: Optional[torch.Tensor] = None,  # shape: (B, S, H, W)
    focal: Optional[torch.Tensor] = None,     # optional, shape: (B, 1)
    downsample_size: Tuple[int, int] = (64, 64)
):
    """
    Estimate global focal (scalar) and shift (3D) per batch from video point maps.

    Args:
        points_all: Tensor (B, S, H, W, 3), point maps in camera space
        mask_all: Optional[Tensor] (B, S, H, W), valid masks
        focal: Optional[Tensor] (B, 1), known focal values per batch
        downsample_size: downsample size for fast estimation

    Returns:
        focal: Tensor (B, 1)
        shift: Tensor (B, 3)
    """
    B, S, H, W, _ = points_all.shape
    device = points_all.device
    dtype = points_all.dtype

    uv = normalized_view_plane_uv(W, H, dtype=dtype, device=device)  # (H, W, 2)
    
    # Downsample UV
    uv_lr = F.interpolate(
        uv.permute(2, 0, 1).unsqueeze(0), downsample_size, mode='nearest'
    )[0].permute(1, 2, 0)  # (H_d, W_d, 2)
    uv_lr_np = uv_lr.cpu().numpy().reshape(-1, 2)

    # Downsample points
    points_all = points_all.reshape(B * S, H, W, 3).permute(0, 3, 1, 2)
    points_all_lr = F.interpolate(
        points_all, downsample_size, mode='nearest'
    ).permute(0, 2, 3, 1)  # (B*S, H_d, W_d, 3)
    points_all_lr = points_all_lr.reshape(B, S, -1, 3)  # (B, S, N, 3)

    if mask_all is not None:
        mask_all = mask_all.reshape(B * S, 1, H, W)
        mask_all_lr = F.interpolate(
            mask_all.float(), downsample_size, mode='nearest'
        ).reshape(B, S, -1) > 0  # (B, S, N)
    else:
        mask_all_lr = torch.ones(
            B, S, downsample_size[0] * downsample_size[1], 
            dtype=torch.bool, device=device
        )

    results_focal, results_shift = [], []

    for b in range(B):
        points_b = points_all_lr[b][mask_all_lr[b]]  # (N_valid, 3)
        
        # Tile UV coordinates for all frames in sequence
        uv_b = np.tile(uv_lr_np[None], (S, 1, 1))
        uv_b = uv_b[mask_all_lr[b].cpu().numpy()]  # (N_valid, 2)

        if points_b.shape[0] < 10:
            results_focal.append(torch.tensor([1], device=device, dtype=dtype))
            results_shift.append(torch.tensor(0, device=device, dtype=dtype))
            continue

        if focal is None:
            shift_np, focal_np = solve_optimal_focal_shift(
                uv_b, points_b.cpu().numpy()
            )
            results_focal.append(
                torch.tensor([focal_np], device=device, dtype=dtype)
            )
        else:
            focal_b = focal[b].item()
            shift_np = solve_optimal_shift(
                uv_b, points_b.cpu().numpy(), focal_b
            )
            results_focal.append(focal[b])

        results_shift.append(torch.tensor(shift_np, device=device, dtype=dtype))

    focal_out = torch.stack(results_focal, dim=0)  # (B, 1)
    shift_out = torch.stack(results_shift, dim=0)  # (B, 3)

    return focal_out, shift_out


def recover_global_focal(
    points_all: torch.Tensor,  # shape: (B, S, H, W, 3)
    mask_all: Optional[torch.Tensor] = None,  # shape: (B, S, H, W)
    downsample_size: Tuple[int, int] = (64, 64)
):
    """
    Estimate global focal (scalar) per batch from video point maps.
    ...
    for b in range(B):
        ...
        if points_b.shape[0] < 10:
            results_focal.append(1)
            continue

        focal_np = solve_optimal_focal(uv_b, points_b.cpu().numpy())
        results_focal.append(
            torch.tensor([focal_np], device=device, dtype=dtype)
        )
...
def mask_aware_nearest_resize(
    ...
    - `nearest_idx`: if return_index is True, nearest neighbor index of the 
       resized map of shape (..., target_height, target_width) for each dimension.
    """
    ...
    indices = torch.arange(
        height * width, dtype=torch.long, device=device
    ).reshape(height, width)
    
    padded_uv = torch.full(
        (height + 2 * padding_h, width + 2 * padding_w, 2), 
    ...
    padded_indices = torch.full(
        (height + 2 * padding_h, width + 2 * padding_w), 
...
def theshold_depth_change(
    depth: torch.Tensor, 
    mask: torch.Tensor, 
    pooler: Literal['min', 'max'], 
    rtol: float = 0.2, 
    kernel_size: int = 3
):
    """
    Detect significant depth changes using pooling operations.
    """
...
def depth_occlusion_edge(
    ...
    """
    Detect occlusion edges in depth map.
    """
    ...
    disp_window = utils3d.torch.sliding_window_2d(
        disp_pad, (kernel_size, kernel_size), 1, dim=(-2, -1)
    )  # [..., H, W, kernel_size ** 2]
    
    mask_window = utils3d.torch.sliding_window_2d(
        mask_pad, (kernel_size, kernel_size), 1, dim=(-2, -1)
    )  # [..., H, W, kernel_size ** 2]

    disp_mean = weighted_mean(disp_window, mask_window, dim=(-2, -1))
    
    # Detect edges based on disparity ratio
    fg_edge_mask = mask & (disp / disp_mean > 1 + tol)
    bg_edge_mask = mask & (disp_mean / disp > 1 + tol)

    # Dilate edges to connect components
    fg_edge_mask = fg_edge_mask & F.max_pool2d(
        bg_edge_mask.float(), kernel_size + 2, stride=1, 
        padding=kernel_size // 2 + 1
    ).bool()
    
    bg_edge_mask = bg_edge_mask & F.max_pool2d(
        fg_edge_mask.float(), kernel_size + 2, stride=1, 
        padding=kernel_size // 2 + 1
    ).bool()
...
def dilate_with_mask(
    input: torch.Tensor, 
    mask: torch.BoolTensor, 
    filter: Literal['min', 'max', 'mean', 'median'] = 'mean', 
    iterations: int = 1
) -> torch.Tensor:
    """
    Dilate values in a tensor using a mask and a specified filter operation.
    """
    kernel = torch.tensor(
        [[False, True, False], [True, True, True], [False, True, False]], 
        device=input.device, dtype=torch.bool
    )
    
    for _ in range(iterations):
        input_window = utils3d.torch.sliding_window_2d(
            F.pad(input, (1, 1, 1, 1), mode='constant', value=0), 
            window_size=3, stride=1, dim=(-2, -1)
        )
        mask_window = kernel & utils3d.torch.sliding_window_2d(
            F.pad(mask, (1, 1, 1, 1), mode='constant', value=False), 
            window_size=3, stride=1, dim=(-2, -1)
        )
        ...
        if filter =='min':
            input = torch.where(
                mask, input, 
                torch.where(mask_window, input_window, torch.inf).min(dim=(-2, -1)).values
            )
        elif filter =='max':
            input = torch.where(
                mask, input, 
                torch.where(mask_window, input_window, -torch.inf).max(dim=(-2, -1)).values
            )
        elif filter == 'mean':
            input = torch.where(
                mask, input, 
                torch.where(mask_window, input_window, torch.nan).nanmean(dim=(-2, -1))
            )
        elif filter =='median':
            input = torch.where(
                mask, input, 
                torch.where(mask_window, input_window, torch.nan).flatten(-2).nanmedian(dim=-1).values
            )
...
def refine_depth_with_normal(
    ...
    """
    Refine depth map using surface normals and intrinsics via iterative optimization.
    """
    device, dtype = depth.device, depth.dtype
    height, width = depth.shape[-2:]
    radius = kernel_size // 2

    # Create grid for derivatives
    duv = torch.stack(
        torch.meshgrid(
            torch.linspace(-radius / width, radius / width, kernel_size, device=device, dtype=dtype), 
    ...
    log_depth_diff = utils3d.torch.sliding_window_2d(
        log_depth, window_size=kernel_size, stride=1, dim=(-2, -1)
    ) - log_depth[..., radius:-radius, radius:-radius, None, None] 
    
    weight = torch.exp(
        -(log_depth_diff / duv.norm(dim=-1).clamp_min_(eps) / 10).square()
    )
    tot_weight = weight.sum(dim=(-2, -1)).clamp_min_(eps)

    uv = utils3d.torch.image_uv(
    ...
    laplacian = (
        weight * (
            (utils3d.torch.sliding_window_2d(grad, window_size=kernel_size, stride=1, dim=(-3, -2)) + 
             grad[..., radius:-radius, radius:-radius, :, None, None]) * 
            (duv.permute(2, 0, 1) / 2)
        ).sum(dim=-3)
    ).sum(dim=(-2, -1))
    ...
    for _ in range(iterations):
        smoothed_log_depth = (
            weight * utils3d.torch.sliding_window_2d(
                log_depth_refine, window_size=kernel_size, stride=1, dim=(-2, -1)
            )
        ).sum(dim=(-2, -1))
        
        update_term = (
            damp * log_depth[..., radius:-radius, radius:-radius] - 
            laplacian + 
            smoothed_log_depth
        ) / (tot_weight + damp)
        
        log_depth_refine[..., radius:-radius, radius:-radius] = (
            0.1 * log_depth_refine[..., radius:-radius, radius:-radius] + 
            0.9 * update_term
        )
