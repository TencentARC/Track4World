...
def align_points_scale_z_shift_multi_anchor(
    points_src: torch.Tensor, 
    points_tgt: torch.Tensor, 
    weight: Optional[torch.Tensor], 
    trunc: Optional[Union[float, torch.Tensor]] = None,
    top_k: int = 10  # 新增参数：选择多少个最好的锚点进行平均
):
    """
    通过选择 top_k 个最佳锚点并平均其结果来稳定地对齐点云。
    """
    ...
    with torch.no_grad():
        ...
        points_src_anchor = torch.stack([zeros, zeros, points_src[anchor_where_batch, anchor_where_n, 2]], dim=-1)
        points_tgt_anchor = torch.stack([zeros, zeros, points_tgt[anchor_where_batch, anchor_where_n, 2]], dim=-1)
        points_src_anchored = points_src[anchor_where_batch, :, :] - points_src_anchor[..., None, :]
        points_tgt_anchored = points_tgt[anchor_where_batch, :, :] - points_tgt_anchor[..., None, :]
        weight_anchored = weight[anchor_where_batch, :, None].expand(-1, -1, 3)

        # 为所有锚点计算 scale 和 loss
        # 注意：为了简化，这里假设 `split_batch_fwd` 不是必须的。如果张量很大，仍需保留。
        all_scales, all_losses, _ = align(points_src_anchored.flatten(-2), points_tgt_anchored.flatten(-2), weight_anchored.flatten(-2), trunc)

    # --- 核心改动：选择 top_k 锚点并平均 ---
    # 为了避免复杂的 scatter 操作，这里使用循环处理每个批次样本，更清晰易懂。
    # 对于性能要求极高的场景，可以考虑使用 torch_scatter 库进行向量化。
    final_scales = torch.zeros(batch_size, device=device, dtype=dtype)
    final_shifts = torch.zeros(batch_size, 3, device=device, dtype=dtype)

    for i in range(batch_size):
        # 找到属于当前样本 i 的所有锚点
        item_mask = (anchor_where_batch == i)
        if not item_mask.any():
            continue

        item_losses = all_losses[item_mask]
        item_scales = all_scales[item_mask]
        
        # 确定实际要平均的锚点数量
        num_anchors_for_item = item_losses.shape[0]
        current_k = min(top_k, num_anchors_for_item)
        if current_k == 0:
            continue
            
        # 找到损失最小的 top_k 个锚点
        _, topk_indices = torch.topk(item_losses, k=current_k, largest=False)
        
        # 取出这些最佳锚点的 scale
        topk_scales = item_scales[topk_indices]
        
        # 计算平均 scale
        avg_scale = topk_scales.mean()
        final_scales[i] = avg_scale

        # --- 为这 top_k 个锚点计算相应的 shift 并平均 ---
        # 找到原始锚点在点云中的位置
        item_anchor_n = anchor_where_n[item_mask][topk_indices]
        
        # 根据每个最佳 scale 计算一个 shift
        src_anchor_z = points_src[i, item_anchor_n, 2]
        tgt_anchor_z = points_tgt[i, item_anchor_n, 2]
        topk_shifts_z = tgt_anchor_z - topk_scales * src_anchor_z
        
        # 平均 shift
        avg_shift_z = topk_shifts_z.mean()
        final_shifts[i, 2] = avg_shift_z

    # Reshape back to original batch shape
    scale = final_scales.reshape(batch_shape)
    shift = final_shifts.reshape(*batch_shape, 3)

    return scale, shift

def align_points_scale_z_shift_stable(
    points_src: torch.Tensor, 
    points_tgt: torch.Tensor, 
    weight: Optional[torch.Tensor],
    trunc: Optional[Union[float, torch.Tensor]] = None,
    ...
    """
    Align `points_src` to `points_tgt` with respect to a shared xyz scale and a z shift
    using a robust weighted least-squares method.

    This function finds scale `s` and z-shift `t_z` that minimize:
    Sum(w_i * || (s * p_src_i + [0, 0, t_z]) - p_tgt_i ||^2)
    It solves the resulting 2x2 linear system in closed form, making it highly stable.
    ...
    batch_shape = points_src.shape[:-2]
    n = points_src.shape[-2]
    
    points_src = points_src.reshape(-1, n, 3)
    points_tgt = points_tgt.reshape(-1, n, 3)
    weight = weight.reshape(-1, n)

    # Add a small epsilon to weights to avoid division by zero if all weights are zero.
    weight = weight + eps 

    # We are solving a 2x2 linear system for [s, t_z]:
    # [[A, B], [B, C]] * [[s], [t_z]] = [[D], [E]]
    #
    # A = Sum(w_i * |p_src_i|^2)
    # B = Sum(w_i * p_src_i.z)
    # C = Sum(w_i)
    # D = Sum(w_i * dot(p_src_i, p_tgt_i))
    # E = Sum(w_i * p_tgt_i.z)

    # C: Sum of weights
    C = torch.sum(weight, dim=1)

    # B: Weighted sum of source z-coordinates
    B = torch.sum(weight * points_src[..., 2], dim=1)

    # E: Weighted sum of target z-coordinates
    E = torch.sum(weight * points_tgt[..., 2], dim=1)

    # A: Weighted sum of squared norms of source points
    A = torch.sum(weight * torch.sum(points_src**2, dim=-1), dim=1)

    # D: Weighted sum of dot products between source and target points
    D = torch.sum(weight * torch.sum(points_src * points_tgt, dim=-1), dim=1)
    ...
    determinant = torch.where(
        torch.abs(determinant) < eps, 
        torch.full_like(determinant, eps) * torch.sign(determinant),
        determinant
    )

    # Solve for scale `s`
    scale = (D * C - B * E) / determinant
    
    # Solve for z-shift `t_z`
    shift_z = (A * E - B * D) / determinant

    # Construct the full shift vector [0, 0, t_z]
    shift = torch.stack([
        torch.zeros_like(shift_z),
        torch.zeros_like(shift_z),
        shift_z
    ], dim=-1)
...
def align_points_scale_and_per_frame_shift(
    points_src: torch.Tensor, 
    points_tgt: torch.Tensor, 
    weight: torch.Tensor,
    trunc: Optional[Union[float, torch.Tensor]] = None
    ...
    """
    Align `points_src` to `points_tgt` by finding a single scale and per-frame shifts.
    
    This function first estimates a robust global scale for the entire video and
    then computes an independent z-shift for each frame, ensuring both efficiency
    and alignment accuracy.

    ### Parameters:
    - `points_src: torch.Tensor` of shape (B, F, N, 3), where B is batch, F is frames, N is points per frame.
    - `points_tgt: torch.Tensor` of shape (B, F, N, 3)
    - `weight: torch.Tensor` of shape (B, F, N)
    - `trunc: Optional[Union[float, torch.Tensor]]` = None
    
    ### Returns:
    - `scale: torch.Tensor` of shape (B, 1)
    - `shift: torch.Tensor` of shape (B, F, 3)
    """
    assert points_src.ndim == 4, "Input tensors must have shape (B, F, N, 3)."
    B, F, N, _ = points_src.shape

    # Step 1: Estimate a single, global scale for the entire video.
    # Flatten the frame dimension to treat all points in a video as one large batch.
    points_src_flattened = points_src.reshape(B, F * N, 3)
    points_tgt_flattened = points_tgt.reshape(B, F * N, 3)
    weight_flattened = weight.reshape(B, F * N)
    
    # Utilize the existing align_points_scale function for a robust global scale.
    scale = align_points_scale(
        points_src=points_src_flattened,
        points_tgt=points_tgt_flattened,
        weight=weight_flattened,
        trunc=trunc
    )
    # The returned scale is of shape (B,), reshape for broadcasting.
    scale = scale.reshape(B, 1, 1)

    # Step 2: Estimate per-frame shifts using the calculated global scale.
    # Apply the global scale to the source points for each frame.
    scaled_points_src = points_src * scale

    # For each frame, calculate the optimal Z-axis shift independently.
    # Reshape to treat each frame as a separate batch for the align_points_z_shift function.
    shift = align_points_z_shift(
        points_src=scaled_points_src.reshape(B * F, N, 3),
        points_tgt=points_tgt.reshape(B * F, N, 3),
        weight=weight.reshape(B * F, N),
        trunc=trunc
    )
    
    # The returned shift is of shape (B * F, 3), reshape back to (B, F, 3).
    shift = shift.reshape(B, F, 3)

    return scale.squeeze(-1), shift

def align_points_scale_xyz_shiftv2(
    points_src: torch.Tensor, 
    points_tgt: torch.Tensor, 
    weight: torch.Tensor,
    trunc: Optional[Union[float, torch.Tensor]] = None
    ...
    # Reshape to treat each frame as a separate batch for the align_points_z_shift function.
    shift = align_points_z_shift(
        points_src=scaled_points_src.reshape(B, N, 3),
        points_tgt=points_tgt.reshape(B, N, 3),
...
def align_depth_affine(depth_src: torch.Tensor, depth_tgt: torch.Tensor, weight: Optional[torch.Tensor], trunc: Optional[Union[float, torch.Tensor]] = None):
    ...
    with torch.no_grad():
        ...
        MAX_ELEMENTS = 2 ** 20
        scale, loss, index = split_batch_fwd(align, MAX_ELEMENTS // n, depth_src_anchored, depth_tgt_anchored, weight_anchored, trunc)   # (anchors,)
...
def align_points_scalev2(points_src: torch.Tensor, points_tgt: torch.Tensor, weight: Optional[torch.Tensor], trunc: Optional[Union[float, torch.Tensor]] = None, max_iters: int = 30, eps: float = 1e-6):
    """
    Align `points_src` to `points_tgt` with respect to a shared xyz scale.
    This is a simplified version of `align_points_scale_xyz_shift` that only computes the scale.
...
def align_points_scale_xyz_shift(points_src: torch.Tensor, points_tgt: torch.Tensor, weight: Optional[torch.Tensor], trunc: Optional[Union[float, torch.Tensor]] = None, max_iters: int = 30, eps: float = 1e-6, exp: int = 20):
    """
    Align `points_src` to `points_tgt` with respect to a shared xyz scale and z shift. 
    It is similar to `align_affine` but scale and shift are applied to different dimensions.
