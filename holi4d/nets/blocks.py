...
def _ntuple(n):
    """Creates a function that repeats a value n times if it's not already an iterable."""
    def parse(x):
...
class CorrBlock:
    ...
    """
    def __init__(self, fmap1, fmap2, corr_levels, corr_radius, mode='area', save_fmap=False):
        ...
        for cnf in block_setting:
            ...
            if cnf.out_channels is not None:
                if cnf.downsample:
                    ...
                    # Convert 2x2 downsampling to 3x3 with dilation to maintain resolution but increase receptive field
                    layers.append(
                        nn.Sequential(
                            norm_layer(cnf.input_channels),
    ...
    def __init__(
        ...
        if use_time_conv:
            self.time_blocks = nn.ModuleList(
                [CNBlock1d(hidden_size, hidden_size, dense=False) for _ in range(time_depth)]
            )
        elif use_time_mixer:
            self.time_blocks = nn.ModuleList(
                [MLPMixerBlock(S=16, dim=hidden_size, depth=1) for _ in range(time_depth)]
            )
        else:
            self.time_blocks = nn.ModuleList(
                [AttnBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attn_class=Attention) for _ in range(time_depth)]
            )

        # Initialize Space Blocks
        if add_space_attn:
            self.space_virtual_blocks = nn.ModuleList(
                [AttnBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attn_class=Attention) for _ in range(space_depth)]
            )
...
class BasicMotionEncoder(nn.Module):
    """Encodes flow and correlation features."""
    ...
    def forward(self, flow, corr):
        ...
        if self.pdim==2 or self.pdim==4:
            ...
            # If flow is already encoded (high dim), concatenate directly
...
class BasicUpdateBlock(nn.Module):
    """Basic GRU-like update block for flow estimation."""
...
class FullUpdateBlock(nn.Module):
    """Update block that includes visibility confidence."""
    def __init__(self, corr_channel, num_blocks, hdim=128, cdim=128, pdim=2, use_attn=False):
        ...
        # Compressor input channels:
        #   dim for flowfeat
        #   dim for ctxfeat
        #   dim for motion_features
        #   pdim for flow (if p=2)
        #   2 for visconf
        
        if pdim==2:
            self.compressor = conv1x1(2*cdim+hdim+2, hdim) 
        else:
            # we concatenate the flow info again, to not lose it
...
class MixerUpdateBlock(nn.Module):
    """Update block using MLP-Mixer for temporal processing."""
    def __init__(self, corr_channel, num_blocks, hdim=128, cdim=128):
...
class CleanUpdateBlock(nn.Module):
    """Clean update block with layer scale."""
...
class RelUpdateBlock(nn.Module):
    """Relative Update Block for 2D flow."""
    def __init__(self, corr_channel, num_blocks, cdim=128, cdim1=128, mdim=128, hdim=128, pdim2d=4, use_attn=True, use_mixer=False, use_conv=False, use_convb=False, use_layer_scale=True, no_time=False, no_space=False, no_ctx=False, use_cxt_corr=False):
        super(RelUpdateBlock, self).__init__()
        self.motion_encoder2d = BasicMotionEncoder(corr_channel, dim=hdim, pdim=pdim2d) # B,hdim,H,W
        self.motion_encoder2d_ctx = BasicMotionEncoder(corr_channel, dim=hdim, pdim=pdim2d) # B,hdim,H,W
    ...
    def forward(self, flowfeat, ctxfeat, visconf, fea_corr, cxt_corr, motion2d, S):
        ...
        flowfeat = self.compressor(torch.cat([flowfeat, ctxfeat, motion_features2d, motion_features2d_ctx, visconf], dim=1))
        for blk in self.refine:
            flowfeat = blk(flowfeat, S)
        flowfeat = self.final_conv(flowfeat)
        return flowfeat
...
class RelUpdate3D(nn.Module):
    """Relative Update Block for 3D flow."""
    def __init__(self, corr_channel, num_blocks, tdim=512, cdim=128, mdim=128, hdim=128, pdim3d=6, use_attn=True, use_mixer=False, use_conv=False, use_convb=False, use_layer_scale=True, no_time=False, no_space=False, no_ctx=False, use_cxt_corr=False, use_prior=False):
        ...
        self.motion_encoder3d = BasicMotionEncoder(corr_channel, dim=mdim, pdim=pdim3d) # B,hdim,H,W
    ...
    def forward(self, flowfeat, fmaps3d_detail, sampled_feature_pre, sampled_feature, flow_3d_prior, pm_corr, motion3d, S):
        ...
        feat = self.compressor(torch.cat([flowfeat, fmaps3d_detail, sampled_feature, sampled_feature_pre, motion_features2d_3d], dim=1))
        for blk in self.refine:
            feat = blk(feat, S)
